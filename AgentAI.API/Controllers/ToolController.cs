...
public class ToolController : ControllerBase
{
    ...
    [HttpPost("execute")]
    public async Task<ActionResult<ApiResponse<ToolExecutionResultDto>>> ExecuteTool([FromBody] ToolExecutionRequest request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Executing tool {ToolType} with action {Action}", request.ToolType, request.Action);

        var result = await _toolExecutionService.ExecuteToolAsync(request, cancellationToken);

        if (!result.Success)
        {
            return BadRequest(new ApiResponse<ToolExecutionResultDto>
            {
                Success = false,
        ...
        return Ok(new ApiResponse<ToolExecutionResultDto>
        {
            Success = true,
            Data = result.Data,
            Message = result.Message
    ...
    public async Task<ActionResult<ApiResponse<ToolExecutionResultDto>>> ApproveTool(Guid executionId, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Approving tool execution {ExecutionId}", executionId);
    ...
    {
        _logger.LogInformation("Rejecting tool execution {ExecutionId}", executionId);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!result.Success)
        {
            return BadRequest(new ApiResponse<PaginatedList<ToolExecutionDto>>
            {
                Success = false,
                Errors = result.Errors,
                Message = result.Message
        ...
        return Ok(new ApiResponse<PaginatedList<ToolExecutionDto>>
        {
            Success = true,
            Data = result.Data
        });

...
public class RetrievalController : ControllerBase
{
    ...
    [HttpPost("search")]
    public async Task<ActionResult<ApiResponse<RetrievalResultDto>>> Search(
        [FromBody] RetrievalRequest request,
        CancellationToken cancellationToken)
    {
        _logger.LogInformation("Retrieval search request from user {UserId}",
            User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)?.Value);

        var result = await _retrievalService.SearchAsync(request, cancellationToken);

        if (!result.Success)
            return BadRequest(new ApiResponse<RetrievalResultDto> { Success = false, Errors = result.Errors, Message = result.Message });

        return Ok(new ApiResponse<RetrievalResultDto> { Success = true, Data = result.Data, Message = result.Message });
    }
}
